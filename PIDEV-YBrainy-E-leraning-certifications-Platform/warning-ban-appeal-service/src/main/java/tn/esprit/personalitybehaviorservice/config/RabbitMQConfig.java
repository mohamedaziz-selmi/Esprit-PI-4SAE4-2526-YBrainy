package tn.esprit.warningbanappealservice.config;
...
public class RabbitMQConfig {
    ...
    @Bean
    public Queue warningEventsQueue(
            @Value("${app.rabbitmq.warning-events-queue}") String queueName,
            @Value("${app.rabbitmq.dead-letter-exchange}") String deadLetterExchange,
            @Value("${app.rabbitmq.warning-events-dead-letter-routing-key}") String deadLetterRoutingKey) {
    ...
    @Bean
    public Queue warningEventsDeadLetterQueue(@Value("${app.rabbitmq.warning-events-dead-letter-queue}") String queueName) {
    ...
    @Bean
    public Queue userDeletedDeadLetterQueue(@Value("${app.rabbitmq.user-deleted-dead-letter-queue}") String queueName) {
        return QueueBuilder.durable(queueName).build();
    }

    @Bean
    public Binding warningEventsBinding(
            @Qualifier("warningEventsQueue") Queue warningEventsQueue,
            @Qualifier("ybrainyEventsExchange") TopicExchange ybrainyEventsExchange,
            @Value("${app.rabbitmq.warning-events-routing-key-pattern}") String routingKeyPattern) {
        return BindingBuilder.bind(warningEventsQueue).to(ybrainyEventsExchange).with(routingKeyPattern);
    }

    @Bean
    public Binding appealEventsBinding(
            @Qualifier("warningEventsQueue") Queue warningEventsQueue,
            @Qualifier("ybrainyEventsExchange") TopicExchange ybrainyEventsExchange,
            @Value("${app.rabbitmq.appeal-events-routing-key-pattern}") String routingKeyPattern) {
        return BindingBuilder.bind(warningEventsQueue).to(ybrainyEventsExchange).with(routingKeyPattern);
    }

    @Bean
    public Binding warningEventsDeadLetterBinding(
            @Qualifier("warningEventsDeadLetterQueue") Queue warningEventsDeadLetterQueue,
            @Qualifier("deadLetterExchange") TopicExchange deadLetterExchange,
            @Value("${app.rabbitmq.warning-events-dead-letter-routing-key}") String routingKey) {
        return BindingBuilder.bind(warningEventsDeadLetterQueue).to(deadLetterExchange).with(routingKey);
    }
