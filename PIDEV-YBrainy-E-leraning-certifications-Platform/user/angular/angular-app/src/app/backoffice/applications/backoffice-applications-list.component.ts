...
type ApplicationView = JobApplication & { offer?: JobOffer; partner?: Partnership };
type SectionView = 'all' | 'evaluate';
type CandidateMatchView = ApplicationView & {
...
@Component({
  selector: 'app-backoffice-applications-list',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule, BackofficeDashboardSidebarComponent],
  templateUrl: './backoffice-applications-list.component.html',
  styleUrl: './backoffice-applications-list.component.css',
  host: { style: 'display:block' },
})
export class BackofficeApplicationsListComponent implements OnInit, OnDestroy {
  activeSection: SectionView = 'all';
  ...
  scoreSummary: ScoreSummary = this.emptySummary();
  ...
  setSection(section: SectionView): void {
    ...
    if (section === 'evaluate') {
      ...
      if (!this.evalOfferId && this.offerId) this.evalOfferId = this.offerId;
    }
  }

  onEvalOfferChange(): void {
    this.evaluationError = null;
    this.evaluationInfo = null;
  ...
  get selectedOffer(): JobOffer | null {
    return this.offers.find((o) => o.id === this.evalOfferId) ?? null;
  }

  get selectedOfferApplicationsCount(): number {
    if (!this.evalOfferId) return 0;
    return this.applications.filter((a) => a.offerId === this.evalOfferId).length;
  }

  get canAnalyze(): boolean {
    return !!this.evalOfferId && !this.isAnalyzing && !this.isApplyingAutoStatus && !this.loading;
  }
  ...
  get views(): ApplicationView[] {
    const offersMap = new Map(this.offers.map((o) => [o.id, o]));
    const partnersMap = new Map(this.partners.map((p) => [p.id, p]));
    return this.applications.map((a) => {
      const offer = offersMap.get(a.offerId);
      const partner = offer ? partnersMap.get(offer.partnershipId) : undefined;
      return { ...a, offer, partner };
    });
  }

  get filtered(): ApplicationView[] {
    const q = this.query.trim().toLowerCase();
    const base = this.views.filter((a) => (this.offerId ? a.offerId === this.offerId : true));
    if (!q) return base;
    return base.filter((a) => {
      const hay = `${a.applicantName} ${a.applicantEmail} ${a.offer?.title ?? ''} ${a.partner?.name ?? ''} ${a.message ?? ''}`.toLowerCase();
  ...
  refreshApplications(): void {
    ...
    this.sub.add(
      this.applicationsStore.listAll().subscribe({
        ...
        error: () => {
          this.loading = false;
          this.loadError = 'Impossible de charger les candidatures depuis le backend.';
        },
      })
    );
  }

  deleteApplication(app: ApplicationView): void {
    const ok = window.confirm(`Supprimer la candidature de "${app.applicantName}" ?`);
    if (!ok) return;
    this.sub.add(
      this.applicationsStore.delete(app.id).subscribe({
        next: () => {
          this.applications = this.applications.filter((x) => x.id !== app.id);
          this.analyzedCandidates = this.analyzedCandidates.filter((x) => x.id !== app.id);
          this.topCandidates = this.topCandidates.filter((x) => x.id !== app.id);
          this.loadError = null;
        },
        error: (err: unknown) => {
          this.loadError = this.buildDeleteErrorMessage(err);
        },
      })
    );
  }

  updateApplicationStatus(app: ApplicationView, nextStatus: JobApplicationStatus): void {
    const current = this.applications.find((item) => item.id === app.id);
    if (!current || current.status === nextStatus) return;
    const previousStatus = current.status;
    this.patchApplicationStatus(app.id, nextStatus);
    this.updatingStatusIds.add(app.id);
    this.loadError = null;

    this.sub.add(
      this.applicationsStore
        .updateReview(app.id, { status: nextStatus, reviewerNotes: app.reviewerNotes ?? null })
        .subscribe({
          next: (updated) => {
            this.updatingStatusIds.delete(app.id);
            this.patchApplicationStatus(updated.id, updated.status);
          },
          error: (err: unknown) => {
            this.updatingStatusIds.delete(app.id);
            this.patchApplicationStatus(app.id, previousStatus);
            this.loadError = this.buildUpdateErrorMessage(err);
          },
        })
    );
  }
    ...
    this.sub.add(
      this.applicationsStore
        ...
        .subscribe({
          ...
          error: (err: unknown) => {
            this.updatingStatusIds.delete(candidate.id);
            this.evaluationError = this.buildUpdateErrorMessage(err);
          },
        })
    );
  }
  ...
  analyzeCandidates(): void {
    ...
    const source = this.views.filter((a) => a.offerId === offer.id);
    if (!source.length) {
      this.analyzedCandidates = [];
      this.topCandidates = [];
      this.scoreSummary = this.emptySummary();
    ...
    const analyzed = source
      .map((candidate) => this.computeCandidateScore(candidate, offerKeywords))
      .sort((a, b) => {
        if (b.compatibilityScore !== a.compatibilityScore) return b.compatibilityScore - a.compatibilityScore;
        return a.applicantName.localeCompare(b.applicantName);
      });

    this.analyzedCandidates = analyzed;
    this.topCandidates = analyzed.slice(0, this.normalizedTopCount());
    this.scoreSummary = this.computeSummary(this.topCandidates, this.minCompatibility);
    this.isAnalyzing = false;

    if (this.autoStatusEnabled) this.applyAutomaticStatuses();
  }
        ...
        this.applicationsStore
          ...
          .subscribe({
            next: (saved) => {
              ...
              this.finishAutoUpdate(update.id, --remaining, failed);
            },
            error: () => {
              failed += 1;
              this.finishAutoUpdate(update.id, --remaining, failed);
  ...
  private finishAutoUpdate(applicationId: string, remaining: number, failed: number): void {
  ...
  private computeCandidateScore(candidate: ApplicationView, offerKeywords: string[]): CandidateMatchView {
    const cvText = this.extractTextFromCv(candidate.cvDataUrl);
    const profileText = `${cvText} ${candidate.message ?? ''} ${candidate.applicantName}`.toLowerCase();
    ...
    const compatibilityScore = Math.min(100, Math.round((matchedKeywords.length / offerKeywords.length) * 100));
  ...
  private computeSummary(candidates: CandidateMatchView[], threshold: number): ScoreSummary {
    ...
    return {
      ...
      max: Math.max(...scores),
      min: Math.min(...scores),
      aboveThreshold: candidates.filter((c) => c.compatibilityScore >= threshold).length,
      withoutCvText: candidates.filter((c) => !c.cvReadable).length,
    };
  }
  ...
  private looksLikeReadableText(content: string): boolean {
    ...
    for (let i = 0; i < sample.length; i++) {
      ...
      if ((code >= 32 && code <= 126) || code === 10 || code === 13 || code === 9) readable += 1;
    }
    return readable / sample.length > 0.7;
  }

  private buildDeleteErrorMessage(err: unknown): string {
    const fallback = 'Suppression impossible pour le moment.';
    if (!(err instanceof HttpErrorResponse)) return fallback;
    if (err.status === 0) return `Backend inaccessible. Verifie que le service tourne sur ${environment.partnerApiBaseUrl}.`;
    if (err.status === 404) return 'Suppression indisponible: endpoint backend non trouve.';
    return fallback;
  }

  private buildUpdateErrorMessage(err: unknown): string {
    const fallback = 'Mise a jour du statut impossible pour le moment.';
    if (!(err instanceof HttpErrorResponse)) return fallback;
    if (err.status === 0) return `Backend inaccessible. Verifie que le service tourne sur ${environment.partnerApiBaseUrl}.`;
    if (err.status === 404) return 'Mise a jour indisponible: endpoint backend non trouve.';
    return fallback;
  }
